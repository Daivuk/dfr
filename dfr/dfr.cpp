...
namespace dfr {
    ...
    void init() {
        ...
        auto ret = FT_Init_FreeType(&g_ttfLibrary);
        assert(!ret);
    ...
    sRenderInfo drawText(
        ...
        while (true) {
            ...
            if (in_formating.align & ALIGNV_TOP)
                ...
                pen.y = (in_outputImage.height - (int)(lines.size() - 1) * lineHeight + (face->size->metrics.ascender >> 6)) / 2;
            else if (in_formating.align & ALIGNV_BOTTOM)
                pen.y = in_outputImage.height + (face->size->metrics.descender >> 6) - ((int)lines.size() - 1) * lineHeight;
